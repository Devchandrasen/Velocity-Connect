"""Screen feed position on the shortened Velocity Connect n78 blade."""
...
from build_n78_donor import AEDT_VERSION, DESIGN_NAME, PROJECT_NAME
...
SETUP_NAME = "TuneFeedPosition"
HEIGHT_SCALE = 0.87
FEED_SCALE = 0.80
FEED_POSITIONS_MM = (0.0, 1.5, 3.0, 4.5, 6.0)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Screen blade feed position at 3.5 GHz."
    )
    parser.add_argument(
        "--project",
        type=Path,
...
def _read_center_result(app: Hfss) -> dict[str, float]:
    ...
    if not data:
        raise RuntimeError("Feed-position setup returned no solution data.")
...
def run(args: argparse.Namespace) -> int:
    ...
    manifest_file = project.with_name(
        project.stem + "_feed_position_manifest.json"
    )
    manifest: dict[str, Any] = {
        ...
        "feed_positions_mm": FEED_POSITIONS_MM,
        "candidates": [],
    }
    ...
    try:
        ...
        for position in FEED_POSITIONS_MM:
            value = f"{position:.12g}mm"
            app[f"spacing_feed{ANTENNA_SUFFIX}"] = value
            ...
            candidate: dict[str, Any] = {
                "feed_position_mm": position,
                "solved": bool(solved),
                "solve_elapsed_seconds": time.perf_counter() - solve_start,
            }
            if solved:
                candidate.update(_read_center_result(app))
            ...
            print(
                f"Feed position {position:.2f} mm: "
                f"S11={candidate.get('s11_db', float('nan')):.3f} dB",
                flush=True,
            )

        if best is None:
            raise RuntimeError("No feed-position candidate solved successfully.")
        app[f"spacing_feed{ANTENNA_SUFFIX}"] = (
            f"{best['feed_position_mm']:.12g}mm"
        )
        app[f"spacing_port{ANTENNA_SUFFIX}"] = "0mm"
        if not app.save_project(file_name=str(project), overwrite=True):
            raise RuntimeError("The feed-position tuning state could not be saved.")

        manifest["best_candidate"] = best
        manifest["status"] = "complete"
        manifest["completed_utc"] = datetime.now(timezone.utc).isoformat()
        manifest_file.write_text(json.dumps(manifest, indent=2), encoding="utf-8")
        print(f"Best feed position: {best['feed_position_mm']} mm")
